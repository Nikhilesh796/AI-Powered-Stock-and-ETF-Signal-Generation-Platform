...
Start-Process .\venv\Scripts\python.exe -ArgumentList "signals/api.py" -WindowStyle Minimized
Write-Host "✅ Signals API started on Port 8000"

# Start Alerts API (Port 8001)
Start-Process .\venv\Scripts\python.exe -ArgumentList "alerts/main.py" -WindowStyle Minimized
Write-Host "✅ Alerts API started on Port 8001"

# Start Backtesting API (Port 8002)
Start-Process .\venv\Scripts\python.exe -ArgumentList "-m uvicorn backtesting.main:app --host 0.0.0.0 --port 8002" -WindowStyle Minimized
